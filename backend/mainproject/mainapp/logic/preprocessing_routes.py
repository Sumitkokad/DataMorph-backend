...
def execute_single_operation_enhanced(df, operation, column, logs, rule_engine):
    ...
    try:
        ...
        if operation.startswith("custom:"):
            custom_prompt = operation.split("custom:", 1)[1]
            if custom_prompt.strip():
                from .llm_logic import LLMAgent
                agent = LLMAgent()
                df = agent.execute_custom_operation(df, column, custom_prompt, logs)
            else:
                logs.append(f"⚠️ {column}: empty custom prompt skipped")
            return df
    ...
    try:
        ...
        # One-hot encoding for categorical columns
            ...
            for col in categorical_cols:
                ...
                # Do not one-hot encode high-cardinality or ID-like columns
                if intent.get('is_id', False) or 'name' in col.lower() or 'customer' in col.lower():
                ...
                if 1 < df[col].nunique() <= MAX_ONEHOT_CATEGORIES:
                    ...
                    skipped_cols.append(f"{col}(cardinality > {MAX_ONEHOT_CATEGORIES})")
            if onehot_cols:
                logs.append(f"🔤 One-hot encoding applied to: {', '.join(onehot_cols)}")
            if skipped_cols:
                logs.append(f"⚠️ Skipped one-hot encoding for: {', '.join(skipped_cols)}")
...
def smart_read_csv(file):
    """Smart CSV reading - handles Django storage paths"""
    logs = []
    try:
        from django.core.files.storage import default_storage
        
        if isinstance(file, str) and file.startswith('uploaded_files/'):
            # Django storage path
...
def save_processed_file_enhanced(df, original_filename, validation_summary):
    """Save processed file"""
    from ..file.download import save_processed_file as save_file
